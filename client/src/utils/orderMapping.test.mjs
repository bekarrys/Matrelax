...
test('cartToOrderItems maps cart entries to admin order items', () => {
  const items = [
    { productId: 'p1', name: 'Royal R1', size: '160×200', fabric: 'Жаккард', extra10cm: true, price: 90000, marketPrice: 97000, quantity: 2 },
  ];
  assert.deepEqual(cartToOrderItems(items), [
    { name: 'Royal R1', modelId: 'p1', size: '160×200', fabric: 'Жаккард', extra10cm: true, quantity: 2, price: 90000, marketPrice: 97000, surcharge: 0 },
  ]);
});
...
test('cartToOrderItems defaults quantity to 1 and extra10cm to false', () => {
  ...
  assert.equal(out[0].marketPrice, null);
  assert.equal(out[0].fabric, null);
});
